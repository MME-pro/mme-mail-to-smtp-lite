...
// Graph is no longer an offered mode, so it publishes no fields - a form
// cannot ask for a tenant ID belonging to a choice that is not on it.
check( 'the app-only fields are not published', ! isset( $fields['ms_tenant_id'], $fields['ms_client_secret'] ), implode( ',', array_keys( $fields ) ) );
check( 'the delegated credentials belong to Own Auth App', 'own_signin' === ( $fields['msoauth_client_id']->depends['value'] ?? '' ), var_export( $fields['msoauth_client_id']->depends ?? [], true ) );
check( 'so does its client secret', 'own_signin' === ( $fields['msoauth_client_sec']->depends['value'] ?? '' ) );
...
// With Graph withdrawn from the selector and the broker switched off,
// Microsoft is back to a single way in - which is the shape this section was
// written for. The regression it guards is that dropping a one-option radio
// used to leave every credential depending on a field the form was no longer
// rendering, so each resolved against nothing, decided it did not apply, and
// hid itself: one radio button removed, the whole form empty.
check( 'the selector is gone', ! isset( $solo['ms_setup_mode'] ), implode( ',', array_keys( $solo ) ) );
check( 'the credentials are still published', isset( $solo['msoauth_client_id'], $solo['msoauth_client_sec'] ), implode( ',', array_keys( $solo ) ) );
check( 'and none of them depends on the vanished field', [] === ( $solo['msoauth_client_id']->depends ?? null ), var_export( $solo['msoauth_client_id']->depends ?? null, true ) );

// The hidden mode contributes nothing to the form either. Ungated - which is
// exactly what happens once the options come down to one - its fields would
// otherwise appear as a tenant ID nobody can use.
check( 'and the hidden app-only mode publishes nothing', ! isset( $solo['ms_tenant_id'] ) );
