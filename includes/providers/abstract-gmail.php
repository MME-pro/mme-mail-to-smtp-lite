...
abstract class Abstract_Gmail extends Abstract_Provider {
	...
	protected function map_error( int $status, string $body ): WP_Error {
		...
		// Google uses `error` for two unrelated things. The OAuth endpoints
		// return a string - {"error":"invalid_grant"} - and the Gmail API
		// returns an object - {"error":{"code":400,"message":"..."}}. Casting
		// the second to a string emits an "Array to string conversion" warning
		// and leaves $reason holding the literal text "Array", so every
		// comparison below quietly stops matching. That is only reachable on a
		// Gmail API error with no errors[0].reason, which is most of them.
		$fallback = $data['error'] ?? '';
