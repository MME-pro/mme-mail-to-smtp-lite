...
foreach ( Microsoft::fields() as $field ) {
	if ( 'ms_setup_mode' === $field->key ) {
		$modes = $field->options;
		check( 'the mode selector defaults to Own Auth App', 'own_signin' === $field->default, (string) $field->default );
	}
}

check( 'the delegated mode is offered', isset( $modes['own_signin'] ), implode( ',', array_keys( $modes ) ) );
// Withdrawn from the selector, but not from the plugin. A connection
// already set to it has to go on sending: taking a working transport away
// in an update is not an acceptable way to narrow a feature set.
check( 'the app-only mode is no longer offered', ! isset( $modes['own_client'] ), implode( ',', array_keys( $modes ) ) );
check( 'but Graph is still registered', Provider_Registry::exists( 'graph' ) );

$plugin->settings->update( [ 'provider' => 'microsoft', 'ms_setup_mode' => 'own_client' ] );
Settings::flush_cache();

$still_graph = new Microsoft( $plugin->settings, $plugin->tokens, $plugin->http );
check(
	'and a connection still set to it keeps sending through Graph',
	'Microsoft 365 (Graph)' === $still_graph->get_label(),
	$still_graph->get_label()
);

$plugin->settings->update( [ 'provider' => '', 'ms_setup_mode' => '' ] );
Settings::flush_cache();
...
check(
	'and the hidden app-only mode publishes no fields to gate',
	! array_key_exists( 'ms_tenant_id', $gated ),
	implode( ',', array_keys( $gated ) )
);
