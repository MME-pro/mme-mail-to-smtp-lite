...
class Smtp extends Abstract_Provider {
	...
	/**
	 * Mail sent through an administrator's own SMTP host is not counted as
	 * metered.
	 *
	 * Nothing is capped, so this no longer decides whether a send is allowed.
	 * It decides which of the two counters a send lands in: `sent` takes
	 * everything, `metered` takes only what went through a connection this
	 * plugin authenticates. That split is what the portal is told about.
	 *
	 * Not part of Provider_Interface, for the reason the other optional statics
	 * are not: the answer is "metered" for everything that does not say
	 * otherwise, so a provider registered by another plugin needs to know
	 * nothing about any of this.
	 */
	public static function is_metered(): bool {
		return false;
	}
