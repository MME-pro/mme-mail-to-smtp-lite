...
class Queue {
	 ...
	 *
	 * The period itself now lives in the settings schema as queue_retention,
	 * because it governs how long a complete message may sit in the database
	 * and that is a decision a site owner is entitled to make. It still
	 * defaults to the seven days this constant held.
	 */
	...
	/**
	 * Drop anything too old to be worth keeping.
	 *
	 * Both states, and the second one matters more than it looks. A failed
	 * row holds a message nobody is waiting for; a pending row holds the
	 * complete message - body, headers, attachments - and used to have no
	 * maximum age at all. A queue that stopped draining, because a
	 * credential expired and nobody noticed, would hold every message it
	 * had accepted since, in the clear, indefinitely. That is other
	 * people's correspondence sitting in a table with no expiry, which is
	 * the one thing a retry buffer must never become.
	 *
	 * A pending message older than the retention period cannot be delivered
	 * usefully anyway. Nobody wants a password reset from three weeks ago.
	 */
	private function discard_expired(): void {
		global $wpdb;

		$table = self::table();
		$days  = max( 1, (int) $this->settings->get( 'queue_retention' ) );
