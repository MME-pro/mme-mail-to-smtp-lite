...
abstract class Abstract_Merged_Provider implements Provider_Interface {
	 ...
	 *
	 * A transport whose mode the selector does not offer contributes nothing.
	 * It remains constructible - that is the whole point of leaving it in
	 * transports() - but a form cannot ask somebody to fill in credentials for
	 * a mode they have no way to choose.
	 *
	...
	public static function fields(): array {
		...
		foreach ( static::transports() as $mode => $class ) {
			...
			// happens the moment the remaining options come down to one, they
			// would ask for a tenant ID belonging to a mode that is not there.
			if ( ! isset( $mode_field->options[ $mode ] ) ) {
				continue;
			}
